...
for /f "delims=" %%i in ('LocalVariables config.ini imageoutformat') do set out=%%i >nul
for /f "delims=" %%i in ('LocalVariables config.ini ffmpeg_finetune_image_advanced custom_args') do set custom_args=%%i >nul

:: Start process

for %%i in (input\*.*) DO ffmpeg -i "%%i" -i "%ol%" -filter_complex "[0:v][1:v] overlay=%x%:%y%:enable='between(t,0,20)'" -q:v %quality% %custom_args% %cd%\output\%%~ni.%outformat%